...
| [模块 2：策略合规检查器](./docs/modules/module2-checker.md) | 制度解析、条款检索、合规分析 |

### 完整流程
| 文档 | 说明 |
|------|------|
| [完整流程使用指南](./docs/PIPELINE_GUIDE.md) | 模块 1+ 模块 2 完整流程、配置文件、使用示例 |
...
├── run_full_pipeline.py         # 完整流程执行脚本（模块 1+ 模块 2）
├── db_config.example.json       # 数据库配置示例
├── docs/                        # 文档目录
│   ├── architecture/            # 架构设计
│   │   └── design-overview.md   # 架构设计总览
│   ├── modules/                 # 模块详细设计
│   │   ├── module1-collector.md # 模块 1 详细设计
│   │   ├── module2-checker.md   # 模块 2 详细设计
│   │   └── PIPELINE_GUIDE.md    # 完整流程使用指南
│   └── testing/                 # 测试相关
...
│   └── it_audit.db              # 主数据库
├── skills/                      # 技能模块
│   ├── 1-audit-item-collector/  # 模块 1: 审计项收集
│   ├── 2-policy-compliance-checker/ # 模块 2: 策略合规检查
...
# 直接指定数据库
python run_full_pipeline.py --db "knowledge-work-plugins/it-audit/data/it_audit.db"

# 包含模块 1（从 Excel 导入）
python run_full_pipeline.py -c formal -e "docs/IT 审计项.xlsx"
```

**详细文档**：[完整流程使用指南](./docs/PIPELINE_GUIDE.md)

### 方式 2：分模块执行

1. 收集审计项（模块 1）:
   ```bash
   python -m knowledge_work_plugins.it_audit.skills.1_audit_item_collector.scripts.collector \
       -d "knowledge-work-plugins/it-audit/data/it_audit.db" \
       "训练材料/审计底稿.xlsx"
   ```
...
2. 检查策略合规（模块 2）:
   ```bash
   python -m knowledge_work_plugins.it_audit.skills.2_policy_compliance_checker.scripts.policy_extractor \
       --db-path "knowledge-work-plugins/it-audit/data/it_audit.db"
   ```

3. 生成分析报告:
   ```bash
   python -m knowledge_work_plugins.it_audit.skills.2_policy_compliance_checker.scripts.analyzers.screening_reporter \
       -d "knowledge-work-plugins/it-audit/data/it_audit.db"
   ```
...
| 版本 | 日期 | 修改内容 |
|------|------|----------|
| v4.0 | 2026-03-03 | 添加完整流程脚本、配置文件支持、DB 选择功能 |
| v3.1 | 2026-03-02 | 数据库路径规则：必须传入 db_path 参数 |
| v3.0 | 2026-03-02 | 文档重组，统一入口索引，更新目录结构 |
| v2.0 | 2026-02-27 | 添加 LLM 校验、场景测试器 |
