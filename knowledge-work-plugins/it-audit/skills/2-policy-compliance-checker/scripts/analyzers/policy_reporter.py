...
class PolicyRequirementReporter:
    """制度要求报告生成器"""
    ...
    def generate_summary(self) -> Dict:
        ...
        for req in requirements:
            dim = req.get('source', {}).get('dimension', '未知维度')
            by_dimension[dim] += 1
        ...
        return {
            ...
            'extract_time': batch_info.get('extract_time', ''),
            'total_items_processed': batch_info.get('items_processed', 0),
            'total_requirements': summary.get('total_requirements_found', 0),
            'by_type': by_type,
    ...
    def print_summary(self) -> None:
        ...
        print(f"提取时间: {summary['extract_time']}")
        print(f"审计项总数: {summary['total_items_processed']}")
        print(f"发现制度要求: {summary['total_requirements']} 条")
        print("\n按类型分布:")
        for req_type, count in summary['by_type'].items():
            if count > 0:
                print(f"  - {req_type}: {count} 条")
        print("\n按维度分布:")
        for dim, count in sorted(summary['by_dimension'].items(), key=lambda x: -x[1]):
            print(f"  - {dim}: {count} 条")
        print("\n置信度分布:")
        print(f"  - 高(≥0.8): {summary['confidence_distribution']['high']} 条")
        print(f"  - 中(0.5-0.8): {summary['confidence_distribution']['medium']} 条")
        print(f"  - 低(<0.5): {summary['confidence_distribution']['low']} 条")
        print("=" * 80)
    ...
    def _generate_html(self) -> str:
        ...
        # 生成统计卡片
        stats_cards = self._generate_stats_cards(summary)
        
        # 生成类型分布图表数据
        type_chart_data = self._generate_type_chart_data(summary['by_type'])
        
        # 生成维度分布表格
        dimension_rows = self._generate_dimension_rows(summary['by_dimension'])
        ...
        html_parts.append(f'            <span>批次号: <strong>{summary["batch_id"]}</strong></span>')
        html_parts.append(f'            <span>提取时间: {summary["extract_time"]}</span>')
        html_parts.append(f'            <span>审计项总数: {summary["total_items_processed"]}</span>')
    ...
    def _generate_requirement_rows(self, requirements: List[Dict]) -> str:
        ...
        for req in requirements:
            ...
            <tr data-type="{req.get('requirement_type', '')}" data-confidence="{confidence_class}">
                ...
                <td>{source.get('item_code', '')}</td>
                <td>{source.get('dimension', '')}</td>
                <td>{clues_html}</td>
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("用法: python policy_reporter.py <result_json_file>")
        sys.exit(1)
