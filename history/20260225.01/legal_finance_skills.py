...
从 knowledge-work-plugins/ 下的 SKILL.md 文件加载专业知识
"""
from typing import Dict, List
from langchain.tools import tool
from skill_loader import create_skill_loader

# 初始化技能加载器，从 SKILL.md 文件加载知识
_skill_loader = create_skill_loader()

# 保留原有的硬编码知识库作为备用（当 SKILL.md 加载失败时使用）
...
def _get_skill_content(category, skill_name):
    """从 SKILL.md 获取技能内容"""
    skill = _skill_loader.get_skill(category, skill_name)
    if skill:
...
def search_legal_knowledge(topic: str) -> str:
    ...
    # 如果找到匹配的技能，从 SKILL.md 获取内容
    if skill_name:
        content = _get_skill_content("legal", skill_name)
        if content:
...
def search_financial_knowledge(topic: str) -> str:
    ...
    # 如果找到匹配的技能，从 SKILL.md 获取内容
    if skill_name:
        content = _get_skill_content("finance", skill_name)
        if content:
...
def get_contract_review_template() -> str:
    ...
    """
    # 尝试从 SKILL.md 获取
    content = _get_skill_content("legal", "contract-review")
    if content:
...
def get_financial_statement_template() -> str:
    ...
    """
    # 尝试从 SKILL.md 获取
    content = _get_skill_content("finance", "financial-statements")
    if content:
...
def get_variance_analysis_template() -> str:
    ...
    """
    # 尝试从 SKILL.md 获取
    content = _get_skill_content("finance", "variance-analysis")
    if content:
